...
int main()
{
    ...
    elemtype tempValue, tempIndex;
    printf("\nList:\n");
    printList(myList, printNode);
    auto begin = GETTIME();
    for (int i = 0; i < amount; i++) {//пробіг по масиву починається з нульового елементу
        tempIndex = i;//вважаємо, що мінімальним є перше число масиву (надалі, підмасиву)
        for (int j = i + 1; j < amount; j++) {//перевірка починається з наступного елементу
            if (getNode(myList, j)->value < getNode(myList, tempIndex)->value)
                tempIndex = j;//якщо елемент, що перевіряється є більшим за те, що ми вважаємо мінімальний, то записується його індекс
            tempValue = getNode(myList, i)->value;//знайдений мінімум масиву ставиться на нульове місце
            getNode(myList, i)->value = getNode(myList, tempIndex)->value;
            getNode(myList, tempIndex)->value = tempValue;
        }
    }
    auto end = GETTIME();
